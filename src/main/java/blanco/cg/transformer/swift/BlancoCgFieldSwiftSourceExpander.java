    ...
    public void transformField(final BlancoCgField cgField,
        ...
        if (BlancoStringUtil.null2Blank(cgField.getAccess()).length() > 0) {
            if (argIsInterface && cgField.getAccess().equals("public")) {
                ...
                // TODO: No access control in Swift? buf.append(cgField.getAccess() + " ");
            }
        }
        ...
        if (cgField.getFinal()) {
            // The "final" qualifier for the field has been enhanced to be expressed as readonly.
            // Mapping where it is not const.
            buf.append("readonly ");
        }
        ...
        if (BlancoStringUtil.null2Blank(cgField.getDefault()).length() > 0) {
            ...
            // If there is no default value, outputs the type.
